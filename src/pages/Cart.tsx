...
import { addToCart, calculatePrice, discoundApplied, removeCartItem } from "../redux/reducer/cartReducer";
    ...
    useEffect(() => {
        ...
        const timeOutId = setTimeout(() => {
            axios.get(`${server}/api/v1/payment/discount?coupon=${couponCode}`, { cancelToken }).then((res) => {
                dispatch(discoundApplied(res.data.discount))
                setIsValidCouponCode(true)
                dispatch(calculatePrice())
            }).catch(() => {
                dispatch(discoundApplied(0))
